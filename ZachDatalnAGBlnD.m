...
load ("ZachDatalnAGBlnD.txt")
size (ZachDatalnAGBlnD)
corr (ZachDatalnAGBlnD)

%%% Splitting The Data into Two
P = 0.80 ;
n = 482
idx = randperm(n)  ;
Train_Data =  ZachDatalnAGBlnD(idx(1:round(P*n)),:)
Test_Data =  ZachDatalnAGBlnD(idx(round(P*n)+1:end),:) 
...
%%% X Variable . . .
X_train = [l_train, D_train];
X_test = [l_test, D_test];
