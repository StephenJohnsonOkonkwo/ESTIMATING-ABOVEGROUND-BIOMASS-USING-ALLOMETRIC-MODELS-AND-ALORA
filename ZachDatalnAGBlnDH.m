...
load ("ZachDatalnAGBlnDH.txt")
size (ZachDatalnAGBlnDH)
corr (ZachDatalnAGBlnDH)

%%% Splitting The Data into Two
P = 0.80 ;
n = 482
idx = randperm(n)  ;
Train_Data =  ZachDatalnAGBlnDH(idx(1:round(P*n)),:)
Test_Data =  ZachDatalnAGBlnDH(idx(round(P*n)+1:end),:) 

%%% Training Data
AGB_train = Train_Data (:, 1);
DH_train = Train_Data (:, 2);
...
%%% X Variable . . .
X_train = [l_train, DH_train];
X_test = [l_test, DH_test];
