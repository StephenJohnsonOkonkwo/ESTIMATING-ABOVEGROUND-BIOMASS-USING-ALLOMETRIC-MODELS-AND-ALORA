...
load ("ZachDatalnAGBlnD2HW.txt")
size (ZachDatalnAGBlnD2HW)
corr (ZachDatalnAGBlnD2HW)

%%% Splitting The Data into Two
P = 0.80 ;
n = 482
idx = randperm(n)  ;
Train_Data =  ZachDatalnAGBlnD2HW(idx(1:round(P*n)),:)
Test_Data =  ZachDatalnAGBlnD2HW(idx(round(P*n)+1:end),:)

%%% Training Data
AGB_train = Train_Data (:, 1);
D2HW_train = Train_Data (:, 2);
...
%%% X Variable . . .
X_train = [l_train, D2HW_train];
X_test = [l_test, D2HW_test];
